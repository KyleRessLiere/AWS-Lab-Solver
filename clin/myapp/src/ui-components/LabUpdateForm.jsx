...
export default function LabUpdateForm(props) {
  const {
    id,
    lab,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...labRecord };
    setSshKey(cleanValues.sshKey);
    setBastionHost(cleanValues.bastionHost);
    setUser1Secret(cleanValues.user1Secret);
    setErrors({});
  };
  const [labRecord, setLabRecord] = React.useState(lab);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Lab, id) : lab;
      setLabRecord(record);
    };
    queryData();
  }, [id, lab]);
  React.useEffect(resetStateValues, [labRecord]);
  const validations = {
    sshKey: [],
    bastionHost: [],
    user1Secret: [],
  ...
  return (
    <Grid
      ...
      {...rest}
      {...getOverrideProps(overrides, "LabUpdateForm")}
    >
      <TextField
        label="Ssh key"
        isRequired={false}
